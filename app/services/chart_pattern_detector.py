...
class TechnicalFeatureExtractor:
    ...
    def extract_all_features(df: pd.DataFrame) -> Dict[str, float]:
        ...
        candle_feats = TechnicalFeatureExtractor._extract_candle_features(open_=df['Open'].values, high=high, low=low, close=close)
        features.update(candle_feats)
        
        # Date Features (Day of Week, etc) assuming index is DatetimeIndex
        if isinstance(df.index, pd.DatetimeIndex):
            features['dayOfWeek'] = float(df.index[-1].dayofweek)
            features['monthOfYear'] = float(df.index[-1].month)
        ...
        if len(close) > 0:
             ...
             features['bodyRatio'] = float(body / rng) if rng > 0 else 0.0
             
             # Wick Ratios
             upper_wick = high[-1] - max(close[-1], df['Open'].values[-1])
             lower_wick = min(close[-1], df['Open'].values[-1]) - low[-1]
             features['upperWickRatio'] = float(upper_wick / rng) if rng > 0 else 0.0
             features['lowerWickRatio'] = float(lower_wick / rng) if rng > 0 else 0.0
             
             # HigherHighs / LowerLows (window 5)
             features['higherHighs'] = 1.0 if len(high) >= 2 and high[-1] > high[-2] else 0.0
             features['lowerLows'] = 1.0 if len(low) >= 2 and low[-1] < low[-2] else 0.0

        # Placeholder for options data (requires separate data source)
        features['oiChange'] = 0.0
        features['callPutRatio'] = 0.0
        features['ivRank'] = 0.0
